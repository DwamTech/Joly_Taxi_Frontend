...
export async function extendAdminSubscription(
  ...
  const response = await fetch(url, {
    ...
    credentials: "include",
    body: JSON.stringify({
      months: newTotalMonths ?? additionalMonths,
      additional_months: additionalMonths,
      months_count: newTotalMonths ?? undefined,
    }),
  });

  if (!response.ok) {
    return parseError(response, "فشل في تمديد الاشتراك");
  }

  const json: AdminActionResponse = await response.json().catch(() => ({}));
  return json?.message || "تم تمديد الاشتراك بنجاح";
}
